...
//--- Lectura del sensor de humedad de tierra ---
    ...
    int porcentajeHumedadTierra = map(valorHumTierra, 400,700,100,0); //Se hace la conversion del dato a porcentaje 400==100% --> 600==0%
...
//--- LEE COMANDOS JSON ENTRANTES
    if (Serial.available()) {
        ...
        if (doc.containsKey("riego")){
            bool entrada = doc["riego"];
            if (entrada){
                digitalWrite(pinRelayBomb, HIGH);
            }else{
                digitalWrite(pinRelayBomb, LOW);
            }
        }
